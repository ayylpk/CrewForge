...
const AGENTS_DIR = path.resolve(import.meta.dir, "..", "..");            // agent/engine
// ⚠️ 9/18 目录重构：引擎深了一层（agent/engine），仓库根要再退一级
...
export async function probeEnvironment(logDir?: string): Promise<EnvironmentReport> {
    ...
    tools.push(await probeTool("mvnw(backend)", process.platform === "win32" ? "cmd.exe" : "./mvnw",
        process.platform === "win32" ? ["/c", "mvnw.cmd", "-v"] : ["-v"], path.join(REPO_ROOT, "backend")));
