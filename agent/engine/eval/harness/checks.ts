...
// ⚠️ 9/18 目录重构：引擎深了一层（agent/engine），eval/harness 到仓库根现在要退四级
...
export function resolveMaven(projectDir: string): { path: string; source: string } | null {
    ...
    const repoWrapper = path.join(REPO_ROOT, "backend", WIN ? "mvnw.cmd" : "mvnw");
    if (fs.existsSync(repoWrapper)) return { path: repoWrapper, source: "仓库 backend/mvnw（最后手段，非生成物自带）" };
    return null;
}
