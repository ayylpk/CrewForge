...
2. **`setup`（前置步骤）——"先造数据再断言"的机器表达。**
   ...
   ⚠️ **占位符必须由本判据自己的 setup 抽出来，且名字完全一致**：`path`/`headers`/`body` 里出现的每个
   `{name}`，都要能在本判据前面某条 setup 的 `extract.name` 里找到**同名**声明——
   写了 `{id}` 就必须有一条 `extract.name = "id"`；抽成 `tid`/`pid` 再去引用 `{id}` 是错的。
   （引擎在批次校验阶段会**直接拒掉**"引用了没有 setup 抽取的占位符"的判据——
   否则它在执行期只会换来 404、被误读成"接口没实现"。）
