...
> ✅ **已经落地（9/18）**：方案 A 只活了一天就被实测否掉 —— 草稿与规划中都是铅笔灰，
> 台账上**肉眼分不开**（用户反馈"草稿/规划中/执行中没颜色区分"）。现在走的是 B：
> `--indigo #7a4fb5` / `--indigo-ink #54307f` + `.stamp-indigo`（见 `DESIGN.md` §1）。
> 第一版选的偏蓝靛 `#33408a` 与晒图青色相重合、亮度比仅 1.36:1，小色点上仍难分，已改紫。
> `--violet` 那个废别名已删，新色是正经 token，不是别名。

我建议 A：免费、够用，而且修的是最要命的那一对。 ← **这条建议已被实测推翻，见上。**
