...
// 用 const 对象而非 enum：enum 是非可擦除语法，Node 的 --experimental-strip-types 不支持，
// 会让所有 import Hub.ts 的脚本（executor 子进程、verify-*.ts）无法用裸 node 直接运行。
export const roles = {
    manager: 0,
    architect: 1,
    backendEngineer: 2,
...
export class TransferStation{
    ...
    // 收件箱是唯一事实来源，wait() 只负责唤醒。每次循环先清信号再等待：
    // 防止"假唤醒"（_signaled=true 但收件箱为空）时调用方循环空转——
    // 无限微任务风暴会饿死 macrotask（定时器/超时全部不触发），表现为整条流水线冻结。
...
// LLM 调用允许受控并发；默认串行，确认稳定后再通过 LLM_CONCURRENCY 放开。
// 每次启动错开 3s，避免多个请求同时建立连接和触发服务端限流。
const configuredConcurrency = Number.parseInt(process.env.LLM_CONCURRENCY ?? "1", 10);
const llmConcurrency = Number.isFinite(configuredConcurrency) && configuredConcurrency > 0
    ? configuredConcurrency
    : 1;
const llmGate = new Semaphore(llmConcurrency);
...
export async function llmWithTimeout<T>(
    ...
    // 诊断心跳（临时）：LLM_HEARTBEAT_MS 设置后，等待期间每隔该毫秒打一行，
    // 用于区分"事件循环被同步阻塞"（心跳停止）与"Promise 挂着但循环正常"（心跳继续）。
    const heartbeatMs = Number.parseInt(process.env.LLM_HEARTBEAT_MS ?? "0", 10);
    const heartbeat = heartbeatMs > 0 ? setInterval(() => {
        console.log(`[hb] ${label} 等待中 t=${((Date.now() - callStart) / 1000).toFixed(0)}s`);
    ...
    const raceTimeout = new Promise<never>((_, reject) => {
        ...
        timer = setTimeout(() => {
            ...
            // 先结束业务 Promise，避免 SDK 的 abort 处理阻塞业务层超时。
            rejectRace(new Error(`${label} 超时 ${Math.round(ms / 1000)}s`));
            // DirectChatDeepSeek 将这个 signal 传到底层 Client，并立即销毁当前请求。
            ctrl.abort();
        }, ms);
    });
    try {
