...
import { retryLLM } from "./retryLLM.ts";                     // 结构化输出失败带错误反馈重试
...
const llmCalls: GraphNode<typeof MessagesState.State> = async (state) => {
  ...
  let response = await llmWithTimeout(
    sig => model.invoke([
      new SystemMessage(pm_system_prompt),
      ...state.messages,
    ], { signal: sig }),
    getModelRequestTimeout("planning"), "PM 对话"
  );
  let parsed = parsePMResponse(response);

  // A bare {"done": true} has no executable requirements. Ask once for the
  // missing contract instead of ending the pipeline with an empty plan.
...
const dispose: GraphNode<typeof MessagesState.State> = async (state) => {
  ...
  // 2. 结构化输出直接拿 tasks（重试 ≤3，失败带校验错误反馈让模型自纠错）
  const parsed = await retryLLM(
    "功能细化",
    getModelRequestTimeout("planning"),
    3,
    (feedback, sig) => disposeModel.invoke([
      new SystemMessage(detail_system_prompt + '\n\n## 功能清单\n' + functionsContent + feedback),
    ], { signal: sig }),
  );
...
const planner: GraphNode<typeof MessagesState.State> = async (state) => {
  ...
  // 2. 结构化输出直接拿规划（重试 ≤3，失败带校验错误反馈让模型自纠错）
  const parsed = await retryLLM(
    "阶段规划",
    getModelRequestTimeout("planning"),
    3,
    (feedback, sig) => plannerModel.invoke([
      new SystemMessage(plan_system_prompt + '\n\n## 已确认的详细功能清单\n' + tasksContent + feedback),
    ], { signal: sig }),
  );
...
export async function runManager(station: TransferStation) {
  ...
  async function messageLoop() {
      ...
      while (true) {
          ...
          if (!next) {
            // 全部阶段完成 → 显式正常退出（退出码 0）。agent 循环是 while(true) 常驻，
            // 不主动退出的话进程会永远挂住，验收要求"完整流水线以退出码 0 收尾"。
            console.log("PM：全部阶段已完成，项目交付，进程正常退出。");
            process.exit(0);
          }
