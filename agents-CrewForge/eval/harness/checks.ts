...
export function checkBuild(exp: ScenarioBuildExpectation, projectDir: string, logDir: string): Promise<CheckResult> {
    ...
    return (async () => {
        ...
        let mavenSource = "";
        if (exp.id.startsWith("frontend")) {
            if (!fs.existsSync(path.join(cwd, "package.json"))) {
                c.status = "blocked";
                c.detail = "前端目录存在但没有 package.json：无法确定构建命令";
        ...
        } else if (exp.id.startsWith("backend")) {
            if (!fs.existsSync(path.join(cwd, "pom.xml"))) {
                c.status = "blocked";
                c.detail = "后端目录存在但没有 pom.xml：无法确定构建命令";
                c.evidence = `ls=${fs.readdirSync(cwd).slice(0, 20).join(",")}`;
                c.finishedAt = nowIso();
                return c;
            }
        ...
        c.logFile = lastLog;
        c.finishedAt = nowIso();
        c.evidence = `${mavenSource ? mavenSource + "\n\n" : ""}${evidences.join("\n\n")}`.slice(-6000);
