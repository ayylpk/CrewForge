...
14. 在明确"不可判定"的判据上反复重跑预演，或改写代码去"凑"一条求值不了的判据；
15. 在同一批失败判据连续两轮没有进展时继续空转（见 5.2）——引擎会在失败集合连续 3 次零变化时强制停手，别等它兜底。
16. 把一整个模块 / 一整个后端目录 / 多个工作项（比如"w1 到 w9"）塞进**一次输出**（见 5.3）——
    输出会被长度上限截断，而**截断的决策整个作废**：那一步什么都不会发生，时间照样烧掉。
...
### 5.1 分清「判不了」和「不对」（★ 这条能省掉几十轮空转）

验收判据有三种结局：**通过 / 失败 / 不可判定**。

- **失败** = 判据能判、服务端答错了 → 回去改代码；
- **不可判定** = 这条判据**求值不了**（形状非法、或执行器还不支持的写法；结果里带
  `[不可判定]` 标记）→ **改代码对它无效**。这是判据侧的问题，只有人能修。

对"不可判定"的判据：**不要**改服务端代码去凑（不存在能让它变绿的实现），
**不要**反复重跑预演想"再看看"（重跑一百次它还是不可判定）。
把它们的 id 与原因如实上报即可。

### 5.2 没有进展要停手（引擎已代你兜底，但别等它）

同一批判据连续预演后、失败集合**一字未变**，说明这段代码没在朝修复方向动。

- **引擎侧已有保险丝**：失败集合连续 **3** 次零变化会被**强制停手**并上报。
  但这道闸是最后一道——真到那一步，本轮的预算已经白烧了。所以请在**连续 2 次**零变化时
  就**主动**停下来，不要换着法子重跑、也不要靠猜去改代码。
- 停下来不是失败：把"通过 / 失败 / 不可判定各多少、卡在哪几条、你判断的原因是什么"如实上报，
  让上层决定是继续修、还是去修判据。

理由：一轮预演要重新起服务、逐条重放几十个判据，成本极高；而"没有进展"通常意味着
**卡点不在你能改的地方**（判据侧问题、环境问题、或需求本身自相矛盾）——
继续烧预算不会让它变得可解。**停手上报不是失败，是把问题交给能解决它的人。**

### 5.3 一步只做一件事（别把输出撑爆）

**你的输出有长度上限。** 一次性写"整个模块 / 整个后端目录 / w1 到 w9"，一定会被截断，
而**截断的决策整个作废**——那一步什么都不会发生，时间和额度照烧（实测单步白烧 155 秒、4 万 token）。

- **一步 = 一个工作项，或 1~2 个文件**；写完就用工具调用落地，再走下一步；
- 发现自己要写的东西"多到一次说不完"，就**先写一部分**、剩下的下一步接着写；
- 收到"你这一步的输出**被 max_tokens 截断了**"时，下一轮**必须拆小**再发——
  把同样的内容原样重发只会再撞一次上限。
