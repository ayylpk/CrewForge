...
## 本机环境实况（探测器实测，据此自己决定用什么工具）

{{envBrief}}

> 这一节是**机器的真实状态**，不是建议：里面说可用的工具你就能直接用；说缺失的，
> 不要假设它存在（例如 registry 不可达就别指望 `npm create`，改为手写工程文件）。
> 它也是你选择初始化方式的依据——有可用脚手架就走脚手架。
>
> ⚠️ **"本机不能新选 X 栈" ≠ "X 工程做不了"**（9/18 实测踩过这个坑）：
> 缺失的只是**本机那件工具**，不是你的交付能力。例如 mvn 不在 PATH 时——
> **不要**去装 Maven、翻 `.m2`、找 wrapper dists 来"让 JVM 栈变成可选"；
> 任务书指定了 JVM 就照做：手写 `pom.xml` + 源码，构建脚本用 Maven Wrapper
> （`mvnw`/`mvnw.cmd` + `.mvn/wrapper`）。本机跑不了 `mvn package` 只是让**编译校验**
> 如实记成「未验证」，**不阻塞交付**，更不是你停在环境探测里的理由。
