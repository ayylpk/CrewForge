...
→ 据此调整 M3：语法闸门降级为**写盘前的廉价预筛**；主线改为 `mvnw -o dependency:build-classpath` 拿真实 classpath → `mvnw compile`（真类型检查）→ 启动 → HTTP 契约。
...
| # | 杠杆 | 预估收益 | 零 LLM 可测性 |
|---|---|---|---|
| C-1 | **提示词装配顺序固化**（稳定段在前：角色/契约/骨架/基线；易变段在后：任务、文件路径、已写文件、反馈），同一 run 内同族调用的前缀**逐字节一致** | 缓存命中率↑（命中部分按缓存价计费） | ✅ 可写单测：断言同族调用前缀哈希一致 |
| C-2 | **小任务砍掉伪代码阶段**（files < 3 直接进代码工位） | 任务级调用数 −30~40%（p9 是 6 任务 6 次伪代码） | ✅ 纯逻辑单测 |
| C-3 | **文件树块改为阶段开始时的快照**（不再随写盘增长）+ 按**字节**上限截断 | 同阶段内后续调用前缀不失效 | ✅ 纯函数单测 |
| C-4 | **工具循环预算化**：预留交付轮次、连续无进展即中止（p9 是 12 轮 × 0 产出 = 最大浪费形态） | 最坏情况成本断崖下降 | ✅ 假模型脚本单测 |
| C-5 | **重试计费而非计次**：闸门打回优先 `edit` 补丁、按成本上限而非固定 3 次封顶 | 返工成本↓ | ✅ 假模型脚本单测 |
| C-6 | 档位调整（D-9：implement/repair→pro）必须与 C-2/C-4 同时上线 | 避免"质量↑但成本↑" | ✅ 配置单测 |
