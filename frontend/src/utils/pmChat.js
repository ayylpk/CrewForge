...
export function pmAnswer(s, text) {
    ...
    if (/进度|到哪|多久|完成了|阶段/.test(text)) {
        if (!total)
            return '还没有任务落到看板上——引擎可能还没跑，或还没派单。可以看右下「执行日志」确认引擎起没起。';
        const lines = [
        ...
        if (failed.length) {
            ...
            if (failed.length > 5)
                lines.push(`（还有 ${failed.length - 5} 个，去任务看板的「失败」列看全）`);
        }
        return lines.join('\n');
    }
    // —— 卡住 / 失败原因 ——
    ...
    if (/文件|产出|写了什么|代码在哪/.test(text)) {
        if (!s.fileCount)
            return '还没有产物落到项目树里（引擎还没写文件，或还没到写代码的阶段）。';
    ...
    if (/下一步|接下来|后面|还差什么/.test(text)) {
        if (failed.length)
            return `先把这 ${failed.length} 个失败任务处理掉（看板点重跑），否则当前阶段过不了收口门。`;
        if (s.pending.length)
            return '先答掉确认门里那道题，引擎在等它才能续跑。';
        if (doing.length) {
        ...
        if (total && doneN < total)
            return `还有 ${todoN} 个待办没派下去，等引擎下一轮调度。`;
        if (total && doneN === total)
            return '当前阶段的任务全完了，引擎会做阶段收口（编译门 + 契约探针），过了就进下一阶段。';
