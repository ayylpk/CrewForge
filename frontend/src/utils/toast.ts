...
function sweepLastShown(now: number) {
  if (lastShown.size <= 50) return
  for (const [k, t] of lastShown) {
    if (now - t >= ERROR_DEDUP_MS) lastShown.delete(k)
  }
}
...
function push(kind: ToastKind, text: string) {
  ...
  if (kind === 'error' || kind === 'warning') {
    if (prev !== undefined && now - prev < ERROR_DEDUP_MS) return
